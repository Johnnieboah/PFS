...
struct LeagueSettingsView: View {
    ...
    // Local state for the TextField (as per last fix attempt)
    ...
    var body: some View {
        VStack {
            Form {
                Section(header: Text("League Identity")) {
                    TextField("League Name", text: $localLeagueName) // Bind to localLeagueName
                        ...
                        .onSubmit {
                            ...
                            // The text in localLeagueName should be preserved.
                            // The .onChange for isLeagueNameFieldFocused will handle updating the binding.
                            isLeagueNameFieldFocused = false
                        }
                        .onChange(of: isLeagueNameFieldFocused) { focused in // Use 'focused' directly for iOS 15+
                            if !focused {
                ...
                Section(header: Text("Playoff Settings")) {
                    ...
                    VStack(alignment: .leading) {
                        ...
                        .onChange(of: maxPlayoffTeamsPerConference) { newValue in // Corrected for iOS 14+
            ...
            Button(action: {
                ...
                if isLeagueNameFieldFocused { // If text field was somehow still focused
                    isLeagueNameFieldFocused = false // Unfocusing it should trigger the .onChange
                }
                // Explicitly update the binding from local state, in case .onChange didn't fire (e.g., user didn't unfocus)
                leagueName = localLeagueName
        ...
        .onAppear {
            ...
            // This ensures that if leagueName already had a value (e.g. editing existing league), it's shown
            localLeagueName = leagueName
            print("LeagueSettingsView appeared. Initial localLeagueName set to: '\(localLeagueName)' from binding: '\(leagueName)'")
            validateAndUpdateWildcardByes()
        }
        .sheet(isPresented: $showingImagePicker) {
            LeagueImagePicker(image: $leagueLogo) // Ensure LeagueImagePicker is correctly defined
        }
    }
